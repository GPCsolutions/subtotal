...
class ActionsSubtotal
{
    ...
    public function printNewFormat(&$object, &$conf, &$langs, $idvar)
    {
        ?>
		 	<script type="text/javascript">
				$(document).ready(function() {
					...
					var label = "<label for='subtotal_line_level'><?php echo $langs->trans('SubtotalLabel');
        ?></label>";
					var select = "<select name='subtotal_line_level'>";
					for (var i=1;i<10;i++)
					{
						select += "<option value="+i+"><?php echo $langs->trans('Level');
        ?> "+i+"</option>";
					}
					select += "</select>";
        ...
        ?>'
						     , '?<?php echo $idvar ?>=<?php echo $object->id;
        ?>&action=add_title_line&level='+$('select[name=subtotal_line_level]').val()
    ...
    public function formBuilddocOptions($parameters)
    {
...
//	if($line->rowid==47) exit;
	        ...
	        if ($conf->global->PRODUCT_USE_UNITS) {
	            $colspan++;
					...
					<tr class="drag drop" rel="subtotal" id="row-<?php echo $line->id ?>" style="<?php
							if ($conf->global->SUBTOTAL_USE_NEW_FORMAT) {
							    if ($line->qty==99) {
							        print 'background-color:#adadcf';
							    } elseif ($line->qty==98) {
							        print 'background-color:#ddddff;';
							    } elseif ($line->qty<=97 && $line->qty>=91) {
							        print 'background-color:#eeeeff;';
							    } elseif ($line->qty==1) {
							        print 'background-color:#adadcf;';
							    } elseif ($line->qty==2) {
							        print 'background-color:#ddddff;';
							    ...
							    if ($line->qty==99) {
							        print 'background-color:#ddffdd';
							    } elseif ($line->qty==98) {
							        print 'background-color:#ddddff;';
							    } elseif ($line->qty==2) {
							        print 'background-color:#eeeeff; ';
							    } else {
							        print 'background-color:#eeffee;' ;
							    }
							}
						   
	        ?>;">
					<td colspan="<?php echo $colspan;
	        ?>" style="font-weight:bold;  <?php echo ($line->qty>90)?'text-align:right':' font-style: italic;' ?> "><?php
    ...
    ?>
				    <td align="center"<?php echo((empty($conf->browser->phone) && ($object->statut == 0  && $user->rights->{$object->element}->creer))?' class="tdlineupdown"':'');
    ?>></td>
					<?php 
}
	        ?>
